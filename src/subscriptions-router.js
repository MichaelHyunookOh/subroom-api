...
subscriptionsRouter
  ...
  .post(requireAuth, jsonParser, (req, res, next) => {
    ...
    SubscriptionService.insertSubscription(req.app.get('db'), newSubscription)
      .then((subscription) => {
        res
          .status(201)
...
subscriptionsRouter
  ...
  .all(requireAuth)
  .all((req, res, next) => {
    SubscriptionService.getById(req.app.get('db'), req.params.subscription_id)
      .then((subscription) => {
        if (!subscription) {
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    const numberOfValues = Object.values(subscriptionToUpdate).filter(Boolean)
      .length;
    if (numberOfValues === 0) {
      return res.status(400).json({
        error: {
